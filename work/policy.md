...
Review shape is exactly `{role, assessed_source_commit, criteria_baseline_commit, rubric_version, verdict, report, model, reasoning_effort, findings}`. `findings` is an array of stable finding references; `model` and `reasoning_effort` are null only when unavailable. Reviewer model/effort choices are configurable in [reviewer-config.json](reviewer-config.json); never silently substitute a weaker unavailable reviewer. Review and evidence `location` paths are repository-root-relative and must resolve locally; HTTPS artifact/report links are allowed for human retrieval. Append evidence, resolution, and review entries; preserve earlier claims and reports rather than rewriting them.

Append review objects in chronological order and preserve earlier reports. The last review entry is the current review disposition: `done` requires that entry to be an independent/domain-owner `ready` review with no findings, the accepted criteria baseline and the rubric version in force at the done transition, and a source revision with no substantive changes before that transition. A later `changes_required` or `unable_to_verify` entry supersedes any earlier ready recommendation. Earlier findings remain blocking until each has explicit resolution evidence in the done transition snapshot; a later resolution cannot repair a past transition. Earlier reports do not need to assess later fixes. A later rubric change leaves historical completion intact and calls for separate current assurance.
...
`ready` needs authorization, clear scope/criteria/methods, and immutable criteria/base refs in the transition commit. `in_progress` needs a named owner and completed required dependencies. `in_review` needs passing evidence for every required criterion. `done` needs the same evidence plus a ready required review, resolutions for blocking findings, and no unfinished required children. A new `done` transition must be committed before it can count complete; a worktree-only `done` row is blocked because its exact transition snapshot cannot be verified. `blocked` needs a concrete reason and next action. `deferred`/`cancelled` need disposition evidence and attributable authority. Returning from `deferred` to `proposed` adds a new decision. Done and cancelled are terminal; create corrective work instead of erasing history. The checker inspects the Git status history and validates transition-time prerequisites, including evidence freshness, review-finding resolutions, and dependency/parent rows from the exact transition commit.
...
Relevant code, accepted criteria, dependencies, tests/fixtures/expected outputs/selection/skips, verification config, checker, review rubric, or CI changes invalidate affected evidence for a new completion claim. For each `(criterion_id, check_id)`, the last appended evidence row is current; earlier rows remain historical and do not gate a new claim once superseded. A stale or unsuccessful latest row still blocks. Historical done claims use the commit that recorded the done transition even when it is already in the comparison base; unavailable history produces `unable_to_check`. Their evidence is interpreted using that completion commit's verification catalog. If a check or review method is later retired or renamed, preserve its historical result and warn that current assurance needs a currently configured check; unknown IDs still block a new completion. Local freshness includes committed, staged, unstaged, and untracked input changes. The checker flags changed paths mechanically; whether a test was weakened is a human/model judgment. Later code does not erase historical completion; current assurance is separate. Bookkeeping-only changes do not rerun expensive behavior checks. For evidence freshness, only appended evidence/resolution/review entries, roadmap status/owner/next action, the same item's execution checkpoint, and new files under `work/reviews/` are treated as report bookkeeping. The checker rejects rewriting or removing existing evidence, resolution, or review entries as bookkeeping. Rerun the inexpensive checker after record updates so submitted claims are assessed. Changes to accepted scope, criteria, decisions, dependencies, or other records still make evidence stale.
