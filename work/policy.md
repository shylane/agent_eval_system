...
Append review objects in chronological order and preserve earlier reports. The last review entry is the current review disposition: `done` requires that entry to be an independent/domain-owner `ready` review with no findings, the accepted criteria baseline and current rubric, and a source revision with no substantive changes before the done transition. A later `changes_required` or `unable_to_verify` entry supersedes any earlier ready recommendation. Earlier findings remain blocking until each has explicit resolution evidence; earlier reports do not need to assess later fixes.
